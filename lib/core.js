...
/* ---- ATOMİK BİRİM GÜNCELLEMESİ ----
   Belgeyi okuyup tamamını geri yazmak (replaceOne) sunucusuz ortamda
   TEHLİKELİ: iki istek aynı anda çalıştığında sonra yazan, öncekinin
   değişikliğini sessizce siler (kayıp güncelleme). Örn. iki personel aynı
   anda talep gönderirse biri kaybolur; ya da yönetici kaydederken araya
   gelen talep, yöneticinin ayarlarını geri alabilir.
   Bu yüzden yazmalar veritabanı tarafında ATOMİK operatörlerle yapılır:
   yalnız ilgili alan/dizi değişir, belgenin geri kalanına dokunulmaz.

   Dönüş: true  = güncellendi
          false = koşul tutmadı (ör. sürüm değişmiş → çakışma)
          null  = Mongo yok (yerel dosya modu) → çağıran eski yolu kullanır */
async function unitUpdate(filterExtra, update) {
    const col = await getCol();
    if (!col) return null;
    try {
        const r = await col.updateOne(Object.assign({ _id: A_DOC }, filterExtra), update);
        return r.matchedCount > 0;
    } catch (e) { console.error('[db] unitUpdate hata:', e.message); return false; }
}
...
module.exports = {
    ...
    aEmpty, loadAsistan, saveAsistan, aFindUser, aReqUser, aCanAccess, unitUpdate,
};
